...
const { processing, neew, wrong } = statusData;
...
describe('Order Controller', () => {
    ...
    chai
      ...
      .end((err, res) => {
        ...
        expect(res.body.message).to.equal(
          'Status of order 2 successfully modified by you'
        );
    ...
    chai
      ...
      .end((err, res) => {
        ...
        expect(res.body.data[0].msg).to.equal(
          'Order status is either New, Processing, Cancelled or Complete'
        );
        done(err);
      });
  });
  ...
  it('should get all the users from database', (done) => {
    chai
      .request(app)
      .get('/api/v1/user/all')
      .set('token', userToken)
      .end((err, res) => {
        ...
        expect(res.body).to.be.an('object');
        expect(res.body.status).to.equal('success');
        expect(res.body.message).to.equal('All users found');
        done(err);
      });
  });
  ...
  it('should get all the users from database', (done) => {
    chai
      ...
      .end((err, res) => {
        ...
        expect(res.body).to.be.an('object');
        expect(res.body.status).to.equal('success');
        expect(res.body.message).to.equal('User 1 successfully found');
        done(err);
      });
  });
});
