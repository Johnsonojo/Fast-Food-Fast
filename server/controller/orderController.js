...
class OrdersController {
  ...
  static getOneOrder(req, res) {
    ...
    db.query(`SELECT * FROM orders WHERE orders.id = ${[orderId]}`).then(
      (result) => {
  ...
  static postAnOrder(req, res) {
    const {
 foodName, foodPrice, qty, address, phone 
} = req.body;
    ...
    db.query(
      'INSERT INTO orders(foodName, foodPrice, qty, address, phone, totalAmount, user_id)'
        + ' values($1, $2, $3, $4, $5, $6, $7) RETURNING *',
      [foodName, foodPrice, qty, address, phone, totalAmount, userId]
    )
  ...
  static updateOrderStatus(req, res) {
    ...
    db.query(`SELECT * FROM orders WHERE orders.id = ${[orderId]}`).then(
      (result) => {
        ...
        return db
          .query('UPDATE orders SET orderstatus = $1 WHERE orders.id = $2', [
            orderStatus,
            orderId
          ])
          .then((result) => {
  ...
  static userOrderHistory(req, res) {
    ...
    db.query('SELECT * FROM orders WHERE user_id=$1', [userId]).then((result) => {
  ...
  /**
   * @description delete an order by a user
   * @param {object} req
   * @param {object} res
   * @returns {object} object
   */
  static deleteAnOrder(req, res) {
    ...
    db.query(
      'DELETE FROM orders where orders.id = $1 and user_id = $2 returning *',
      [orderId, userId]
    )
      .then((deleteResult) => {
